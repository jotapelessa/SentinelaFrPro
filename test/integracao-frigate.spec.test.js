...
test('AC-031: Persistência Atômica Bidirecional e Invalidação de Cache de Câmeras @spec:AC-031', () => {
  ...
  assert.ok(camerasApiCode.includes('/api/streams') && camerasApiCode.includes('name'), 'cameras.py deve registrar stream dinamicamente no go2rtc');
  ...
  assert.ok(camerasApiCode.includes('/api/config/save'), 'cameras.py deve gravar configuração autoritativa no Frigate');
